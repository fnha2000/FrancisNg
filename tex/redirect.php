<?php
    ...
    if (strcmp($action,"logout") == 0):
        ...
        session_destroy();
        setcookie("TextExchange[id]", $userinfo['id'], time()-600);
        setcookie("TextExchange[email]", $userinfo['email'], time()-600);
        header("Location: login.php");
        exit;
    elseif (isset($_POST['diffid'])):
        setcookie("TextExchange[id]", $_POST['id'], time()-600);
        setcookie("TextExchange[email]", $_POST['email'], time()-600);
        header("Location: login.php");
        exit;
    ...
    elseif (strcmp($_SESSION['status'], "wrongemail") == 0):
        ...
        show_end();
        exit;
    elseif (strcmp($_SESSION['status'], "unauth") == 0):
        $_SESSION['status'] = "";
        show_header();
        ?>
        <p>Sorry, you are not authorized to access this part of the site.</p>
        <?php
        show_end();
    endif;
    ...
    function show_header() {
        ...
        <html>
            <head>
                <title>TextExchangeV2</title>
                <link rel="stylesheet" type="text/css" href="css/style.css" />
            </head>
            <body><section class="main">
                    <header>TextExchangeV2</header>
        <?php
    }
